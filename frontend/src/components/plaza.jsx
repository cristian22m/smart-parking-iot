...
function Plaza({ id, estado, auto, position }) {
  ...
  const sizeClasses = isHorizontal ? 'w-32 lg:w-36 h-20 lg:h-24' : 'w-28 lg:w-32 h-32 lg:h-36';

  return (
    <div
      className={`${sizeClasses} m-1 lg:m-2 rounded-lg shadow-lg flex flex-col items-center justify-center relative ${
        estado ? 'bg-red-500' : 'bg-green-400'
      } border-2 lg:border-4 border-white transition-all duration-300`}
    >
      {estado && auto ? (
        <img src={auto} alt="auto" className={`w-full h-full object-contain ${getImageClass()}`} />
      ) : (
